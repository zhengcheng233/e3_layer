...
from ..data import CondensedDataset
...
def get_config(spec=None):
    ...
    config.early_stopping_lower_bounds = {"LR": 1e-5}
    config.early_stopping_delta = {"training_loss":0.5}
    ...
    if config.md == False:
        ds = CondensedDataset(data.path)
        shifts = ds.statistics(['energy-per-species-mean_std'],stride=config.n_stride)[0][0][0:num_types]
    else:
